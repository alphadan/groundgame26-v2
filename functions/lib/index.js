    ...
    try {
        ...
        rolesSnap.forEach((doc) => {
            ...
            if (d.role)
                rolesSet.add(d.role);
            if (d.group_id)
                groupIdsSet.add(d.group_id);
            if (d.county_id)
                countiesSet.add(d.county_id);
            if (d.district_id)
                districtsSet.add(d.district_id);
            if (d.area_id)
                areasSet.add(d.area_id);
            if (d.precinct_id)
                precinctsSet.add(d.precinct_id);
        });
        ...
        else if (["area_chair", "candidate", "state_rep_district"].includes(primaryRole)) {
            precinctsSet.add("ALL");
        }
        const finalAccess = {
        ...
        const filteredAreas = aSnap.docs
            ...
            .filter((a) => primaryRole === "developer" ||
        ...
        const filteredPrecincts = pSnap.docs
            .map((d) => ({ ...d.data(), id: d.id }))
            .filter((p) => primaryRole === "developer" ||
...
// ================================================================
export const queryVoters = onRequest({ cors: true, region: "us-central1" }, async (req, res) => {
    try {
        ...
        const isValidTable = lowerSql.includes("from \`${VOTER_TABLE}\`") ||
...
export const getVotersByPrecinct = onCall(async (request) => {
    ...
    if (!precinctCode) {
        throw new HttpsError("invalid-argument", "The function must be called with a precinctCode.");
    }
...
export const getVotersByPrecinctV2 = onCall({
    ...
    try {
        ...
        console.log(`Loaded ${rows.length} voters for precinct "${precinctCode}" (normalized: "${normalizedPrecinct}")`);
        return { voters: rows };
    }
    catch (error) {
...
export const queryVotersDynamic = onCall({
  ...
  `;
    ...
    // === Mail Ballot ===
    if (filters.mailBallot !== undefined &&
...
export const getMessageIdeas = onCall(async (request) => {
    ...
    try {
        ...
        const partyValue = data.political_party === "R"
            ? "Republican"
        ...
        const applyOrFilter = (queryRef, field, value) => {
            ...
            return queryRef.where(Filter.or(Filter.where(field, "==", value), Filter.where(field, "==", null)));
        };
        // Apply filters
        ...
        throw new HttpsError("internal", error.message || "Failed to fetch templates.");
    }
});
// ================================================================
    ...
    try {
        ...
        rolesSnap.forEach((doc) => {
            ...
            if (d.role)
                roles.add(d.role);
            if (d.group_id)
                groupIds.add(d.group_id);
            if (d.county_id)
                counties.add(d.county_id);
            if (d.district_id)
                districts.add(d.district_id); // NEW
            if (d.area_id)
                areas.add(d.area_id);
            if (d.precinct_id)
                precincts.add(d.precinct_id);
        });
        ...
        logger.info(`✅ [getUserProfile] UID: ${uid} | Role: ${primaryRole} | Districts: ${finalAccess.districts.length}`);
...
// ================================================================
export const syncOrgRolesToClaims = onDocumentWritten("org_roles/{docId}", async (event) => {
    ...
    try {
        ...
        else if (primaryRole === "area_chair" ||
        ...
        console.log(`✅ Claims & Profile synced for UID: ${uid}. Role: ${primaryRole}. Areas: ${JSON.stringify(finalAreas)}`);
        return null;
    }
    catch (error) {
...
async function getJurisdictionAccess(countyId, type, value) {
    ...
    precinctSnap.forEach((doc) => {
        ...
        if (data.area_id)
            areaIds.add(data.area_id);
        if (data.party_rep_district)
            districtIds.add(data.party_rep_district);
    });
    return {
        counties: [countyId],
...
export const updateDistrictLeadership = onCall(async (request) => {
    ...
    try {
        ...
        if (leaders && leaders.length > 2) {
            throw new HttpsError("invalid-argument", "A district cannot have more than two leaders.");
        }
        await db
...
export const adminCreateArea = onCall(async (request) => {
    ...
    if (!cleanId || !data.name || !data.area_district) {
        throw new HttpsError("invalid-argument", "Missing required fields: id, name, or area_district.");
    }
    try {
...
export const adminImportAreas = onCall(async (request) => {
    ...
    if (!Array.isArray(areas) || areas.length === 0) {
        throw new HttpsError("invalid-argument", "Expected a non-empty array of areas.");
    }
...
export const adminCreateMessageTemplate = onCall(async (request) => {
    ...
    try {
        ...
        const templateData = {
            ...
            turnout_score_general: data.turnout_score_general === "all"
                ? null
                : data.turnout_score_general || null,
            has_mail_ballot: data.has_mail_ballot === "all" ? null : data.has_mail_ballot || null,
            tags: tagsArray,
            active: data.active ?? true,
...
// functions/index.js
export const adminGenerateResourceUploadUrl = onCall({ cors: true }, async (request) => {
    // ADD LOGGING HERE
    logger.info("Incoming Upload Request Data:", request.data);
    const { title, category, description, fileName, county_id, area_id, precinct_id, county_code, area_code, precinct_code, scope, } = request.data;
...
export const onResourceUploaded = onObjectFinalized(async (event) => {
    ...
    if (!fileData.name.startsWith("resources/"))
        return;
    try {
        ...
        await db
            ...
            .doc(customDocId)
            .set(resourceData);
        logger.info(`Successfully indexed ${customDocId} with nested location map.`);
    }
    catch (error) {
...
// ================================================================
export const onCampaignResourceDeleted = onDocumentDeleted("campaign_resources/{resourceId}", async (event) => {
    ...
    if (!storagePath) {
        logger.warn(`No storage_path found for resource ${event.params.resourceId}`);
        return;
    }
    try {
...
// ================================================================
export const getResourcesByLocation = onCall({ cors: true }, async (request) => {
    const { county, area, precinct } = request.data;
    // 1. Validation: We need at least a County ID to start
    if (!county || !county.full) {
        throw new HttpsError("invalid-argument", "A valid County selection is required.");
    }
    try {
        ...
        logger.info(`Found ${allResources.length} resources for ${precinct?.name || area?.name || county.name}`);
        return {
            success: true,
            categorized,
        ...
        throw new HttpsError("internal", "Failed to retrieve campaign materials.");
    }
});
// ================================================================
...
export const adminCreateUser = onCall({ cors: true }, async (request) => {
    ...
    const { email, display_name, preferred_name, phone, role, group_id, county_id, district_id, area_id, precinct_id, active, jurisdiction_type, jurisdiction_value, tempPassword, } = request.data;
    // 2. Data Validation
    const isCandidate = role === "candidate";
    if (!email || !display_name || !role || !phone) {
        throw new HttpsError("invalid-argument", "Missing core user identity fields.");
    }
    ...
    const adminPermsDoc = await db
        ...
        .get();
    const allowedRoles = adminPermsDoc.data()?.[adminRole]?.allowed_to_create || [];
    if (adminRole !== "developer" && !allowedRoles.includes(role)) {
        throw new HttpsError("permission-denied", `Your role (${adminRole}) is not authorized to provision ${role}s.`);
    }
    try {
        ...
        if (formattedPhone.length !== 12) {
            throw new HttpsError("invalid-argument", "Valid 10-digit US mobile number required for MFA.");
        }
...
// ================================================================
export const adminResetUserPassword = onCall({ cors: true }, async (request) => {
...
export const adminSetGoal = onCall({ cors: true }, async (request) => {
    ...
    if (!request.auth) {
        throw new HttpsError("unauthenticated", "You must be an admin to set goals.");
    }
    // 2. Destructure data from request.data
    const { precinct_id, precinct_name, targets, ai_narratives, cycle, county_id, area_id, } = request.data;
    // Validation
...
export const searchVotersByPhoneV2 = onCall({
    ...
    if (normalizedPhone.length < 10) {
        throw new HttpsError("invalid-argument", "Please enter at least a 10-digit phone number");
    }
    const table = VOTER_TABLE;
    ...
    try {
        ...
        console.log(`Phone search for "${normalizedPhone}" returned ${rows.length} results`);
        return { voters: rows };
    }
    catch (error) {
...
export const adminCreateOrgRole = onCall({ cors: true }, async (request) => {
    ...
    if (!role || !county_id || !group_id) {
        throw new HttpsError("invalid-argument", "Missing required fields: role, county_id, or group_id.");
    }
    try {
        ...
        throw new HttpsError("internal", error.message || "Failed to create position.");
    }
});
// ================================================================
...
export const adminVacateOrgRole = onCall({ cors: true }, async (request) => {
    ...
    try {
        return await db.runTransaction(async (transaction) => {
            ...
            if (!roleSnap.exists)
                throw new Error("Role position not found.");
            const roleData = roleSnap.data();
            ...
            if (targetUid) {
                ...
                const newAccess = {
                    ...
                    areas: [...new Set(remainingRoles.map((r) => r.area_id))].filter(Boolean),
                    precincts: [
                        ...new Set(remainingRoles.map((r) => r.precinct_id)),
                    ].filter(Boolean),
                };
                // Determine new primary role (usually the highest rank or 'base' if none left)
                const newRoleLabel = remainingRoles.length > 0 ? remainingRoles[0].role : "base";
                transaction.update(userRef, {
                    role: newRoleLabel,
                    access: newAccess,
    ...
    try {
        ...
        if (roleData.uid) {
            ...
            const newAccess = {
                ...
                precincts: [...new Set(activeRoles.map((r) => r.precinct_id))].filter(Boolean),
            };
...
export const adminCreateSurvey = onCall(async (request) => {
    ...
    try {
        ...
        throw new HttpsError("internal", error.message || "Failed to create survey");
    }
});
// ================================================================
...
export const adminUpdateSurvey = onCall({ cors: true }, async (request) => {
    ...
    if (!updates.survey_id || !updates.name || !updates.jsonPath) {
        throw new HttpsError("invalid-argument", "Missing required fields in updates");
    }
    try {
        await db
            .collection("surveys")
        ...
        throw new HttpsError("internal", error.message || "Failed to update survey");
    }
});
// ================================================================
//  CREATE OR EDIT SURVEY META THROUGH ADMIN
// ================================================================
export const adminCreateOrUpdateSurvey = onCall({ cors: true }, async (request) => {
    // 1. Auth Check
    if (!request.auth) {
        throw new HttpsError("unauthenticated", "You must be logged in.");
    }
    ...
    if (!rawId || !data.name?.trim() || !data.jsonPath?.trim()) {
        throw new HttpsError("invalid-argument", "Missing required fields: survey_id, name, or jsonPath.");
    }
...
export const syncFCMTopics = onDocumentUpdated("users/{uid}", async (event) => {
    ...
    if (tokens.length === 0) {
        logger.info(`User ${uid} updated preferences but has no active device tokens.`);
        return;
    }
...
export const cleanupExpiredInteractions = onSchedule({
    ...
    try {
        ...
        logger.info(`Successfully deleted ${snapshot.size} expired interactions.`, {
